...
import 'models/_new/profile_model.dart';
import 'package:flutter/foundation.dart';
import 'models/_new/customer_model.dart';
...
abstract class AppData {
  ...
  /// Authentication Data
  $Customer $user;
  Future<void> setUser($Customer customer);
...
class _AppDataImpl implements AppData {
  ...
  $Customer get $user {
    if ($isAuthenticated) {
      return Hive.box<$Customer>('\$customers').values.first;
    } else {
      return null;
    }
  }

  Future<void> setUser($Customer customer) async {
    Hive.box<$Customer>('\$customers').add(customer);
    await customer.save();
  }
  ...
  bool get $isAuthenticated => Hive.box<$Customer>('\$customers').isNotEmpty;
  ...
  Future _loadCache() async {
    ...
    _accessToken = preferences.getString('access_token');

    try {
      _coordinates = LatLng(preferences.getDouble('latitude'),
